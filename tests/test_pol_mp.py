...
def test_inference_ap3_atom_model():
    """
    Run inference for the AP3 atom-type and induced-dipole models and validate outputs against a stored reference.
    
    Loads pretrained AtomTypeParamModel and AtomInducedDipoleModel, compiles the induced-dipole model, and predicts on two LR water dimer molecules (batch_size=2). Prints the prediction result and asserts each predicted tensor matches the saved reference file at ../debug_ap3_atom_model_inference.pt within an absolute tolerance of 1e-6.
    """
    atpm = AtomModels.ap3_atomtype_mpnn.AtomTypeParamModel(
        use_GPU=False,
...
def train_ap3_atom_model():
    """
    Train an AP3 atom-induced-dipole model on a local atomic dataset and save the trained model.
    
    This function builds an atomic dataset (spec_type 10) from the repository test data, constructs an AP3 AtomTypeParamModel and an AtomInducedDipoleModel, configures OpenMP threads for parallelism, and runs a full training run (configured for 100 epochs and batch_size 16). The trained model is written to the repository models path.
    """
    ds = atomic_datasets.atomic_module_dataset(
...
def test_lmdb_dataset_creation():
    """
    Verify an LMDB-backed atomic dataset can be created and exposes expected item fields.
    
    Creates an LMDB dataset using a pretrained atom-type HFVR model, asserts the dataset contains at least one entry, and verifies that a retrieved item includes node features (`x`), edge indices (`edge_index`), volume ratios (`volume_ratios`), and valence widths (`valence_widths`).
    """
...
def test_train_with_lmdb():
    """
    Train an AtomInducedDipoleModel end-to-end using an LMDB-backed dataset.
    
    This test verifies that an AtomInducedDipoleModel can be constructed from a pretrained AtomTypeParamModel and trained when the dataset is stored in LMDB. It exercises LMDB usage, precomputation of HFVR/volume-ratio features, and a short training loop on a small dataset to confirm the end-to-end training pipeline runs without errors.
    """
    # Load pre-trained atomtype model
    atpm = AtomModels.ap3_atomtype_mpnn.AtomTypeParamModel(
        use_GPU=False,
...
def debug_ap3_atom_model():
    """
    Create a frozen AP3 induced-dipole model from pretrained components and run a single forward pass on a saved debug batch.
    
    Builds an AtomTypeParamModel from the configured pretrained atom-type path, constructs an InducedDipoleModel that uses a pretrained AtomMPNN, loads a debug batch tensor from ../debug_batch.pt, runs the model on that batch, and prints the model and the forward-pass output.
    """
    atpm = AtomModels.ap3_atomtype_mpnn.AtomTypeParamModel(
        use_GPU=False,
        ignore_database_null=True,
...
def test_mtp_elst_dimers():
    """
    Compute and print monomer and dimer electrostatic and induction energies for two predefined water dimer test molecules.
    
    This test loads a pretrained atom-type model and an induced-dipole model, calls the model's dimer prediction routine to obtain per-molecule monomer electrostatic energies, dimer electrostatic energies, and induction energies, then prints those values for each test dimer.
    """
    atpm = AtomModels.ap3_atomtype_mpnn.AtomTypeParamModel(
        use_GPU=False,
        ignore_database_null=True,
...
def test_train_frozen_dipole_with_pretrained_atom_mpnn():
    """
    Train an InducedDipoleModel initialized with a pretrained AtomMPNN and verify that only dipole-related layers are trainable while charge and multipole layers remain frozen.
    
    Constructs an atomic dataset, loads a pretrained AtomTypeParamModel for HFVR, and builds an InducedDipoleModel using a supplied AtomMPNN checkpoint. Confirms via assertions that parameters whose names contain `dipole_update_layers` and `dipole_readout_layers` require gradients, and that parameters containing `charge_update_layers`, `charge_readout_layers`, `qpole1_update_layers`, `qpole2_update_layers`, and `qpole_readout_layers` do not require gradients. After verification, performs a short training run to exercise the training loop with the frozen/ trainable configuration.
    
    Raises:
        AssertionError: If any of the expected trainable or frozen layer checks fail.
    """
    # Load dataset
    ds = atomic_datasets.atomic_module_dataset(
        root=data_path,
