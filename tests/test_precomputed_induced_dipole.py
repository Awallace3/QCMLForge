...
def create_test_atomtype_model():
    """
    Create a small AtomTypeParamMPNN configured for tests.
    
    Returns:
        An AtomTypeParamMPNN instance configured with small dimensions and two parameters (hfvr and vw): n_message=2, n_neuron=32, n_embed=4, n_params=2, r_cut=5.0.
    """
...
def atomtype_hfvr_model():
    """
    Provide an AtomTypeParamMPNN for tests, loading from a checkpoint if present or creating a small test model otherwise.
    
    If a checkpoint exists at `atp_mpnn_path`, the function reconstructs an AtomTypeParamMPNN from the saved config and loads a cleaned state_dict (removing any `_orig_mod.` prefixes). If no checkpoint is found, it returns a newly created lightweight test model. The returned model is set to evaluation mode and has gradients disabled.
    
    Returns:
        AtomTypeParamMPNN: A pre-configured, non-trainable AtomTypeParamMPNN suitable for use in tests.
    """
...
def test_forward_pass_equivalence(atomtype_hfvr_model):
    """
    Verify numeric equivalence of model outputs when HFVR/VW values are provided precomputed versus computed on the fly.
    
    Asserts that predicted charges, dipoles, and quadrupoles match between the precomputed and traditional computation paths within tolerances rtol=1e-5 and atol=1e-6.
    """
