...
def test_tf_converted_atom_model_loads():
    ...
    v_pt = am_pt.predict_qcel_mols([ref_water.get_fragment(0)], batch_size=1)[0]
    pp(v_pt[0])
    pp(v_pt[1])

    for i in range(5):
        model_path = os.path.join(project_root, f"models/ap2_tf/atom_models/atom{i}.pt")
        ...
        v = atom_model.predict_qcel_mols([ref_water.get_fragment(0)], batch_size=1)[0]
        pp(v[0])
        pp(v[1])
...
def test_tf_converted_predict_water_dimer_single():
    ...
    pair_model.set_pretrained_model(
        ap2_model_path=os.path.join(project_root, "models/ap2_ensemble/ap2_0.pt"),
    ...
    atom_model_path = os.path.join(project_root, "models/ap2_tf/atom_models/atom0.pt")
    pair_model_path = os.path.join(project_root, "models/ap2_tf/pair_models/pair0.pt")
    # Load models
    pair_model = APNet2_AM_Model(
        atom_model=None,
        ignore_database_null=True,
