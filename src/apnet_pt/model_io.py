...
A MACE-fused model cannot use v2: its foundation backbone is a licensed
third-party artifact that must stay out of the checkpoint, so v3 stores a
digest and a locator for it instead of its weights.  v3 is therefore written
only by :func:`create_mace_checkpoint_v3`; v2 remains the default everywhere
else.
...
# The one state-dict prefix v3 is allowed to drop, because those tensors are
# reloaded from the external artifact rather than stored.
