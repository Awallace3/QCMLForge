...
def _cliff2_rackers_induction(
    *,
    ...
    except ImportError as error:  # pragma: no cover - depends on tree contents
        raise ImportError(
            "induction_mode='cliff2-rackers' needs rackers_thole_induction from "
            "apnet_pt.AtomPairwiseModels.mtp_mtp, which this tree does not "
            "provide; use 'ap3-no-correction' or install a build that carries "
            "the CLIFF2 kernels"
        ) from error
