...
#Not really used anywhere but weights module
def reference_cn() -> torch.tensor:
    """
    Provide a fixed matrix of per-element reference coefficients.
    
    Returns:
        torch.Tensor: 2D tensor with seven columns; each row corresponds to an element entry ordered from None, H, He, …, Lr. Unused coefficient slots are marked with -1.0.
    """
    return torch.tensor(
        [
